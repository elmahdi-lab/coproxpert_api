...
func (uq *UserQuery) loadCredential(ctx context.Context, query *CredentialQuery, nodes []*User, init func(*User), assign func(*User, *Credential)) error {
	...
	for _, n := range neighbors {
		fk := n.user_credential
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_credential" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_credential" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
